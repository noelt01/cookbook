...
def test_model_structure_table_exists(db_inspector):
    assert db_inspector.has_table("ingredients")
...
def test_model_structure_column_data_types(db_inspector):
    ...
    assert isinstance(columns["amount"]["type"], String)
    assert isinstance(columns["measurement"]["type"], String)
...
def test_model_structure_column_constraints(db_inspector):
    ...
    assert any(
        constraint["name"] == "ingredient_name_length_check" for constraint in constraints
    )
    assert any(
        constraint["name"] == "ingredient_amount_length_check" for constraint in constraints
    )
    assert any(
        constraint["name"] == "ingredient_measurement_length_check" for constraint in constraints
    )
"""
...
def test_model_structure_column_lengths(db_inspector):
    ...
    assert columns["amount"]["type"].length == 5
    assert columns["measurement"]["type"].length == 15
